  ...
  return (
    <div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-4 py-8 text-center text-primary/50">No categories yet.</td></tr>
            ) : (
              categories.map((c) => (
                <tr key={c.id} className="border-b border-lavender-light last:border-0 hover:bg-lavender-light">
                  <td className="px-4 py-3 font-medium text-ink">{c.name}</td>
                  <td className="px-4 py-3 text-ink/60">{c._count?.products ?? 0}</td>
      ...
      <ConfirmDialog
        ...
        description={`Delete "${deleteTarget?.name}"? Products using it will keep their other data but lose this category.`}
      />
